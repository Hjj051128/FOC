...
//PID
PIDController vel_loop_M0(2, 0, 0, 100000, 0);
...
void DFOC_Vbus(float power_supply)
{
  ...
  pinMode(pwmB, OUTPUT);
  pinMode(pwmC, OUTPUT);
  ledcAttach(pwmA, 30000, 8);  //pwm引脚, 频率, 精度
  ledcAttach(pwmB, 30000, 8);  //pwm引脚, 频率, 精度
  ledcAttach(pwmC, 30000, 8);  //pwm引脚, 频率, 精度
  Serial.println("完成PWM初始化设置");

  //AS5600
  S0_I2C.begin(8,9, 400000UL);
